...
func (sv *SettingsView) Show() {
	...
	win.Resize(fyne.NewSize(560, 700))
	...
	resetBtn := widget.NewButtonWithIcon("Reset to Defaults", theme.ViewRefreshIcon(), func() {
		dialog.ShowConfirm("Reset Settings", "Restore all settings to their default values?", func(yes bool) {
			if !yes {
				return
			}
	...
	content := container.NewBorder(
		...
		container.NewVScroll(container.NewVBox(
			pathsCard, windowCard, behaviorCard, scanCard, privCard,
		)),
	)
